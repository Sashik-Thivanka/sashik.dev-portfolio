...
export default function InfiniteMenu({ items = [], scale = 1.0, className }) {
  const canvasRef = useRef(null)
  const rafSketchRef = useRef(null)
  const [activeItem, setActiveItem] = useState(null)
  const [isMoving, setIsMoving] = useState(false)

  const safeItems = useMemo(() => (items?.length ? items : defaultItems), [items])
  ...
  useEffect(() => {
    ...
    try {
      sketch = new InfiniteGridMenu(canvas, safeItems, handleActiveItem, setIsMoving, (sk) => sk.run(), scale)
      rafSketchRef.current = sketch
    } catch (e) {
      console.error(e)
    }
    ...
    return () => {
      ...
      sketch?.destroy?.()
      rafSketchRef.current = null
    }
  }, [safeItems, scale])

  return (
    <div className={['infinite-menu', className].filter(Boolean).join(' ')} style={{ position: 'relative', width: '100%', height: '100%' }}>
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} />

      {/* Optional overlay (desktop only via CSS media query) */}
      {activeItem && (
        <>
