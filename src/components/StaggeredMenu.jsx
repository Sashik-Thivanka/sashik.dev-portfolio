...
export default function StaggeredMenu({
  ...
  const animateIcon = useCallback(opening => {
    ...
    iconTweenRef.current.to(
      plusH,
      { rotate: opening ? 45 : 0, duration: opening ? 0.45 : 0.3, ease: opening ? 'power4.out' : 'power3.inOut' },
      0
    )
    iconTweenRef.current.to(
      plusV,
      { rotate: opening ? -45 : 90, duration: opening ? 0.45 : 0.3, ease: opening ? 'power4.out' : 'power3.inOut' },
      0
    )
  }, [])
  ...
  const panelClass = `${className ? `${className} ` : ''}staggered-menu-wrapper${isFixed ? ' fixed-wrapper' : ''}`
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#101010', '#1f1f1f']
          const arr = [...raw]
      ...
      <header className="staggered-menu-header" aria-label="Main navigation header">
        <a className="sm-logo" aria-label="Go to top" href={logoHref} onClick={closeMenu}>
          {logoText}
        </a>
